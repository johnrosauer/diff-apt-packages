...
# Helper to run a command in the C locale (locale-independent)
c_run() {
    LC_ALL=C command "$@"
}
...
if [ -n "$LOCAL_MANIFEST" ]; then
    ...
    $reader "$LOCAL_MANIFEST" | awk '{print $1}' | sed -E 's/:[a-zA-Z0-9_-]+$//' | c_run sort -u > "$DEFAULT_LIST_FILE"
...
if [ -z "$DEFAULT_SOURCE" ]; then
    ...
    awk '{print $1}' "${TMP_DIR}/manifest.raw" | sed -E 's/:[a-zA-Z0-9_-]+$//' | c_run sort -u > "$DEFAULT_LIST_FILE"
fi

# Step 3: Extract current packages
CURRENT_LIST_FILE="${TMP_DIR}/current.txt"
dpkg-query -W -f='${db:Status-Status} ${Package}\n' | grep -vE '^(not-installed|config-files) ' | cut -d' ' -f2 | sed -E 's/:[a-zA-Z0-9_-]+$//' | c_run sort -u > "$CURRENT_LIST_FILE"
...
if [ "$KEEP_VERSIONS" = true ]; then
    ...
    c_run comm -23 "$CURRENT_LIST_FILE" "$DEFAULT_LIST_FILE" > "$ADDED_FILE"
    c_run comm -13 "$CURRENT_LIST_FILE" "$DEFAULT_LIST_FILE" > "$REMOVED_FILE"
else
    ...
    normalize_and_map() {
        ...
        paste -d' ' "$norm_tmp" "$src" | c_run sort -k1,1 > "$dest_mapped"
        awk '{print $1}' "$dest_mapped" | c_run sort -u > "$dest_names"
    }
    ...
    c_run comm -23 "${TMP_DIR}/cur_names.txt" "${TMP_DIR}/def_names.txt" > "${TMP_DIR}/add_names.txt"
    c_run comm -13 "${TMP_DIR}/cur_names.txt" "${TMP_DIR}/def_names.txt" > "${TMP_DIR}/rem_names.txt"
    
    # Map back to original package names using join
    c_run join -o 1.2 "${TMP_DIR}/cur_mapped.txt" "${TMP_DIR}/add_names.txt" | c_run sort -u > "$ADDED_FILE"
    c_run join -o 1.2 "${TMP_DIR}/def_mapped.txt" "${TMP_DIR}/rem_names.txt" | c_run sort -u > "$REMOVED_FILE"
fi
